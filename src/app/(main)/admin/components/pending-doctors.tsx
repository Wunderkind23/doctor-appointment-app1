'use client';

import { useState } from 'react';
import { format } from 'date-fns';
import { Check, X, User, Medal, FileText, Loader2 } from 'lucide-react';

import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Separator } from '@/components/ui/separator';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from '@/components/ui/dialog';
import { Badge } from '@/components/ui/badge';
import { toast } from 'sonner';
import { SingleDocAttributeI, useFetchDocs } from '../hooks/useFetchDocs';
import { handleApiError } from '@/lib/errors/axios';
import { useUpdateDocStatus } from '../hooks/useUpdateDocStatus';

export function PendingDoctors() {
  const [selectedDoctor, setSelectedDoctor] = useState<SingleDocAttributeI | null>(null);

  const { data: docs, refetch } = useFetchDocs('unverified');

  const { mutate: updateDocStatus, isPending } = useUpdateDocStatus();

  const handleViewDetails = (doctor: SingleDocAttributeI) => {
    setSelectedDoctor(doctor);
  };

  const handleCloseDialog = () => {
    setSelectedDoctor(null);
  };

  const handleUpdateStatus = async (
    doctorId: number,
    status: 'VERIFIED' | 'REJECTED'
  ): Promise<void> => {
    if (status === 'VERIFIED') {
      updateDocStatus(doctorId, {
        onSuccess: () => {
          toast('Doctor Verified Successfully');
          handleCloseDialog();
          refetch();
        },
        onError: (error: unknown) => {
          handleApiError(error);
        },
      });
    }

    if (status === 'REJECTED') {
      handleCloseDialog();
    }
  };

  return (
    <div>
      <Card className="bg-muted/20 border-brand-900/20">
        <CardHeader>
          <CardTitle className="text-xl font-bold text-foreground">
            Pending Doctor Verifications
          </CardTitle>
          <CardDescription>Review and approve doctor applications</CardDescription>
        </CardHeader>

        <CardContent>
          {docs?.result?.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              No pending verification requests at this time.
            </div>
          ) : (
            <div className="space-y-4">
              {docs?.result.map((doctor) => (
                <Card
                  key={doctor.id}
                  className="bg-background border-brand-900/20 hover:border-brand-700/30 transition-all"
                >
                  <CardContent className="p-4">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className="bg-muted/20 rounded-full p-2">
                          <User className="h-5 w-5 text-brand-400" />
                        </div>
                        <div>
                          <h3 className="font-medium text-foreground">{`${doctor?.user.firstName} ${doctor?.user.lastName}`}</h3>
                          <p className="text-sm text-muted-foreground">
                            {doctor.specialization} • {doctor.yearsOfExperience} years experience
                          </p>
                        </div>
                      </div>

                      <div className="flex items-center gap-2 self-end md:self-auto">
                        <Badge
                          variant="outline"
                          className="bg-amber-900/20 border-amber-900/30 text-amber-400"
                        >
                          Pending
                        </Badge>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleViewDetails(doctor)}
                          className="border-brand-900/30 hover:bg-muted/80"
                        >
                          View Details
                        </Button>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ))}
            </div>
          )}
        </CardContent>
      </Card>

      {selectedDoctor && (
        <Dialog open={!!selectedDoctor} onOpenChange={handleCloseDialog}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-foreground">
                Doctor Verification Details
              </DialogTitle>
              <DialogDescription>
                Review the doctor&apos;s information carefully before making a decision
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-6 py-4">
              <div className="flex flex-col md:flex-row gap-6">
                <div className="space-y-1 flex-1">
                  <h4 className="text-sm font-medium text-muted-foreground">Full Name</h4>
                  <p className="text-base font-medium text-foreground">{`${selectedDoctor?.user?.firstName} ${selectedDoctor?.user?.firstName}`}</p>
                </div>

                <div className="space-y-1 flex-1">
                  <h4 className="text-sm font-medium text-muted-foreground">Email</h4>
                  <p className="text-base font-medium text-foreground">
                    {selectedDoctor?.user?.email}
                  </p>
                </div>

                <div className="space-y-1 flex-1">
                  <h4 className="text-sm font-medium text-muted-foreground">Application Date</h4>
                  <p className="text-base font-medium text-foreground">
                    {format(new Date(selectedDoctor.createdAt), 'PPP')}
                  </p>
                </div>
              </div>

              <Separator className="bg-brand-900/20" />

              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <Medal className="h-5 w-5 text-brand-400" />
                  <h3 className="text-foreground font-medium">Professional Information</h3>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6">
                  <div className="space-y-1">
                    <h4 className="text-sm font-medium text-muted-foreground">Specialty</h4>
                    <p className="text-foreground">{selectedDoctor.specialization}</p>
                  </div>

                  <div className="space-y-1">
                    <h4 className="text-sm font-medium text-muted-foreground">
                      Years of Experience
                    </h4>
                    <p className="text-foreground">{selectedDoctor.yearsOfExperience} years</p>
                  </div>

                  {/* 
                  <div className="space-y-1 col-span-2">
                    <h4 className="text-sm font-medium text-muted-foreground">Qualifications</h4>
                    <p className="text-foreground foregroundspace-pre-line">
                      {selectedDoctor.qualifications}
                    </p>
                  </div> */}
                </div>
              </div>

              <Separator className="bg-brand-900/20" />

              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <FileText className="h-5 w-5 text-brand-400" />
                  <h3 className="text-foreground font-medium">Registration Number</h3>
                </div>
                <p className="text-muted-foreground">{selectedDoctor.medicalLicense}</p>
              </div>
            </div>

            {/* {loading && <BarLoader width="100%" color="#36d7b7" />} */}

            <DialogFooter className="flex sm:justify-between">
              <Button
                variant="destructive"
                onClick={() => handleUpdateStatus(selectedDoctor.id, 'REJECTED')}
                // disabled={loading}
                className="bg-red-600 hover:bg-red-700"
              >
                <X className="mr-2 h-4 w-4" />
                Reject
              </Button>

              <Button
                onClick={() => handleUpdateStatus(selectedDoctor.id, 'VERIFIED')}
                // disabled={loading}
                className="bg-brand-600 hover:bg-brand-700"
              >
                {isPending ? (
                  <>
                    <Loader2 className="animate-spin" />
                    Approving
                  </>
                ) : (
                  <>
                    <Check className="mr-2 h-4 w-4" />
                    Approve
                  </>
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
    </div>
  );
}
